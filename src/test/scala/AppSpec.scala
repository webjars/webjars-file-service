...
object AppSpec extends ZIOSpecDefault:
  ...
  def spec = suite("All tests")(
    ...
    ).provide(Client.default, App.serverLayer, App.jarCacheLayer) @@ TestAspect.sequential, // todo: random server port and shared server (can't do that because Server.install duplicates routes)

  ).provide(Client.default, Scope.default, App.jarCacheLayer)
